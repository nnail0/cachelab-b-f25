...
So, the first attempt made was to simply try blocking tactics that worked well with the other 32x32 and 64x64 sizes. First, we will try to block 8x8 (as the size allows within the rectangle), and fill with the straightforward blocking technique--as in, no U-shaped access patterns. We will leave diagonals alone since they do not align as consistently. That simple access pattern already works very well, yielding 2426 misses. 
